...
const MemberSendPaymentLink = ({
  ...
  useEffect(() => {
    ...
    if (startDateNext) {
      ...
      // choose valid date
      nextStartDate = calculatedDate < today ? today : calculatedDate;
    }
...
useEffect(() => {
  ...
  // If already selected date is available, keep it
  if (
    formik.values.start_date &&
    isDateAvailable(new Date(formik.values.start_date))
  ) {
    return;
  }
...
}, [clubSlotsData, clubSlotsLoading]);
...
export default MemberSendPaymentLink;
