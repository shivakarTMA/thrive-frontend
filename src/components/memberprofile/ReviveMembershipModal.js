...
const ReviveMembershipModal = ({
  ...
  const formik = useFormik({
    initialValues: {
      ...
      no_of_days: "",
      remarks: "",
      revivel_type: "SUBSCRIPTION",
      pdfFile: null,
    },

    validationSchema: Yup.object({
      no_of_days: Yup.number()
        .typeError("Revive days must be a number")
        .required("Revive days is required")
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        ...
        formData.append(
          ...
          values.subscription_booking_id,
        );
        formData.append("no_of_days", values.no_of_days);
        formData.append("remarks", values.remarks);
        formData.append("revivel_type", values.revivel_type);

        // PDF File
        formData.append("pdfFile", values.pdfFile);

        const response = await authAxios().put("/revivel/expired/service", formData, {
          headers: {
      ...
      <div className="max-w-sm w-full">
        <form onSubmit={formik.handleSubmit}>
          <div className="bg-white p-6 rounded-t shadow-lg">
            <h2 className="text-xl font-bold text-black mb-2">Revive Membership</h2>
            {/* Revive Days */}
            <div>
              <label className="mb-2 block">
                Number of revive days
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    formik.setFieldValue("no_of_days", cleanValue);
                  }}
          ...
          <div className="flex justify-end gap-2 bg-white px-2 py-3 border-t rounded-b">
            <button
              type="button"
              onClick={() => setReviveMembership(false)}
...
export default ReviveMembershipModal;
