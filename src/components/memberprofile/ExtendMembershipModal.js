...
const ExtendMembershipModal = ({
  setExtendMembershipModal,
  membershipData,
  fetchMemberById,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        ...
        setExtendMembershipModal(false);
      ...
      <div className="max-w-sm w-full">
        <form onSubmit={formik.handleSubmit}>
          <div className="bg-white p-6 rounded-t shadow-lg">
            <h2 className="text-xl font-bold text-black mb-2">Extend Membership</h2>
            {/* Extension Days */}
            <div>
          ...
          <div className="flex justify-end gap-2 bg-white px-2 py-3 border-t rounded-b">
            <button
              type="button"
              onClick={() => setExtendMembershipModal(false)}
