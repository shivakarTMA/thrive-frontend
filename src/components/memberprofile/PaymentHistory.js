...
const PaymentHistory = ({ details, setLoading }) => {
  ...
  const confirmSend = async (mode) => {
    ...
    try {
      ...
      await authAxios().post(
        `/member/send/invoice/email/${details?.id}`,
        payload,
      );
  ...
  const fetchInvoice = async (order) => {
    ...
    try {
      const payload = {
        order_id: order.order_id,  // confirm the exact key here, maybe order.id in your data
        order_type: order.order_type,
      };

      const res = await authAxios().post("/invoice/generate", payload);

      // Assuming the API returns invoice JSON in res.data.invoice or directly in res.data
      const invoice = res.data?.data;

      setInvoiceData(invoice); // Save invoice data in state
      setInvoiceModal(true);   // Open the modal
    } catch (err) {
      ...
      {invoiceModal && invoiceData && (
        <InvoiceModal
          ...
          onClose={() => {
            ...
            setInvoiceData(null); // Clear data on modal close
          }}
          data={invoiceData}  // Pass fetched data here
        />
      )}
    </>
  );
