...
const KYCSubmission = ({ details, setLoading }) => {
    ...
    const fetchKycDocuments = async () => {
      try {
        ...
        if (response.data.status && response.data.data) {
          ...
          const corporateId = data.find((d) => d.document_type === "CORPORATE_ID");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload or update Corporate ID (CORPORATE_ID)
            ...
            return (
              <div key={type} className="bg-gray-50 rounded-lg p-6 w-full">
                <label className="block text-lg font-semibold text-gray-700 mb-4">
                  {config.label} {type === "corporateId" && !corporateId ? "" : "*"}
                </label>
