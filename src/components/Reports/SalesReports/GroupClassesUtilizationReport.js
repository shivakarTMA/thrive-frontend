...
const GroupClassesUtilizationReport = (props) => {
  ...
  const handleExportGroupClassesReport = async () => {
    try {
      setLoading(true);
      ...
      const response = await authAxios().get("/report/group/class/download", {
        params,
        responseType: "blob",
      });
      ...
      link.setAttribute("download", "Group_Classes_Report.xlsx");
      ...
      toast.success("Group classes report downloaded successfully!");
    } catch (error) {
      console.error(error);

      toast.error("Failed to download group classes report.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex gap-3 mb-4 items-center justify-between">
        ...
        {!ALLOWED_ROLES.includes(userRole) && (
          <div className="w-full max-w-[170px]">
            <button
              onClick={handleExportGroupClassesReport}
...
export default IsLoadingHOC(GroupClassesUtilizationReport);
