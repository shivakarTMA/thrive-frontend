...
const LeadSourceReport = (props) => {
  ...
  const fetchLeadSourceReport = async (currentPage = page) => {
    try {
      const params = {
        page: currentPage,
        limit: rowsPerPage,
      ...
      const res = await authAxios().get("/report/lead/source/list", { params });
  ...
  const handleExportLeadSourceReport = async () => {
    try {
      setLoading(true);
      ...
      const response = await authAxios().get("/report/lead/source/download", {
        params,
        responseType: "blob",
      });
      ...
      link.setAttribute("download", "Lead_Source_Report.xlsx");
      ...
      toast.success("Lead source report downloaded successfully!");
    } catch (error) {
      console.error(error);

      toast.error("Failed to download lead source report.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex gap-3 mb-4 items-center justify-between">
        ...
        {!ALLOWED_ROLES.includes(userRole) && (
          <div className="w-full max-w-[170px]">
            <button
              onClick={handleExportLeadSourceReport}
              ...
              {activeMember.length ? (
                activeMember.map((row, index) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-4">{row?.total_leads}</td>
                    <td className="px-2 py-4">{row?.new_count}</td>
...
export default IsLoadingHOC(LeadSourceReport);
