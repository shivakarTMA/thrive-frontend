...
const ReferralReport = (props) => {
  ...
  const fetchReferralReport = async (currentPage = page) => {
    try {
      const params = {
        page: currentPage,
        limit: rowsPerPage,
      ...
      const res = await authAxios().get("/report/referral/list", {
        params,
      });
  ...
  const handleExportReferralReport = async () => {
    try {
      setLoading(true);
      ...
      const response = await authAxios().get("/report/referral/download", {
        params,
        responseType: "blob",
      });
      ...
      link.setAttribute("download", "Referral_Report.xlsx");
      ...
      toast.success("Referral report downloaded successfully!");
    } catch (error) {
      console.error(error);

      toast.error("Failed to download referral report.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="page--content">
      ...
      <div className="flex items-end justify-between gap-2 mb-5">
        <div className="title--breadcrumbs">
          <p className="text-sm">
            {`Home > Reports > Operations Reports > Referral Report`}
          </p>
          <h1 className="text-3xl font-semibold">Referral Report</h1>
        </div>
      </div>
          ...
          <table className="w-full text-sm text-left text-gray-500">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50">
              <tr>
                ...
                <th className="px-2 py-4 min-w-[100px] text-center">Converted (Y/N)</th>
              ...
              {activeMember.length ? (
                activeMember.map((row, index) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-2">{row.sales_rep_name ? row.sales_rep_name : "--"}</td>
                    {/* <td className="px-2 py-2">{row.leadSource}</td> */}
                    <td className="px-2 py-2">{row.referrer_name}</td>
