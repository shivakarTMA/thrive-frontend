...
const SalesIncentiveReport = (props) => {
  ...
  const fetchSalesIncentiveReport = async (currentPage = page) => {
    try {
      const params = {
        page: currentPage,
        limit: rowsPerPage,
      ...
      const res = await authAxios().get("/incentive/policy/sales", {
        params,
      });
 ...
 useEffect(() => {
  ...
  const handleDownloadSalesIncentiveReport = async () => {
    try {
      setLoading(true);
      ...
      const response = await authAxios().get("/incentive/policy/sales/download", {
        params,
        responseType: "blob",
      });
      ...
      link.setAttribute("download", "Sales_Incentive_Report.xlsx");
      ...
      toast.success("Sales incentive report downloaded successfully!");
    } catch (error) {
      console.error(error);

      toast.error("Failed to download Sales incentive report.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="page--content">
      ...
      <div className="flex items-end justify-between gap-2 mb-5">
        <div className="title--breadcrumbs">
          <h1 className="text-3xl font-semibold">Sales Incentive Report</h1>
        </div>
      </div>
      ...
      <div className="flex gap-3 mb-4 items-center justify-between">
        ...
        {!ALLOWED_ROLES.includes(userRole) && (
            <div className="w-full">
              <button
                onClick={handleDownloadSalesIncentiveReport}
      ...
      <div className="w-full p-3 border bg-white shodow--box rounded-[10px]">
              ...
              {leaderboardCallLogs.map((row, index) => (
                <tr key={`${row.staff_id}-${index}`} className="bg-white border-b hover:bg-gray-50 border-gray-200">
                  ...
                  <td className="  px-2 py-4 text-center">
                    ₹{row?.collection ? formatIndianNumber(row?.collection) : 0}
                  </td>
        ...
        <Pagination
          ...
          onPageChange={(newPage) => {
            setPage(newPage);
            fetchSalesIncentiveReport(newPage);
          }}
        />
      </div>
    </div>
...
export default IsLoadingHOC(SalesIncentiveReport);
