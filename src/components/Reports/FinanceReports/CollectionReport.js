...
const CollectionReport = (props) => {
  ...
  const handleExportData = async () => {
    try {
      ...
      const response = await authAxios().get("/report/collection/list/download", {
        params,
        responseType: "blob",
      });
      ...
      console.error(error);
      toast.error("Failed to download Collection Report.");
    } finally {
      setLoading(false);
    }
  };
...
export default IsLoadingHOC(CollectionReport);
