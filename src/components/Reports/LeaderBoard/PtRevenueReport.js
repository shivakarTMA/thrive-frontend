...
const PtRevenueReport = (props) => {
  ...
  const fetchPtRevenueReportLeaderboard = async (currentPage = page) => {
    try {
      const params = {
        page: currentPage,
        limit: rowsPerPage,
      ...
      const res = await authAxios().get("/leaderboard/sales/pt/revenue", {
        params,
      });
 ...
 useEffect(() => {
    if (clubFilter !== null) {
      setPage(1);
      fetchPtRevenueReportLeaderboard(1);
    }
      ...
      <div className="grid grid-cols-2 gap-3 mb-5 p-3 border bg-white shodow--box rounded-[10px]">
        <div className="border rounded-[5px] overflow-hidden w-full">
          <div className="flex gap-1 justify-center bg-[#F1F1F1] p-4 py-3">
            <div className="text-lg font-bold text-center">Total New Sales</div>
          </div>
          <div>
            <p className="text-3xl font-bold p-2 text-center py-5">
              ₹{stats?.totalNewSales ? formatIndianNumber(stats?.totalNewSales) : 0}
            </p>
          </div>
        </div>
        <div className="border rounded-[5px] overflow-hidden w-full">
          <div className="flex gap-1 justify-center bg-[#F1F1F1] p-4 py-3">
            <div className="text-lg font-bold text-center">Total Renewals</div>
          </div>
          <div>
            <p className="text-3xl font-bold p-2 text-center py-5">
            ₹{stats?.totalRenewals ? formatIndianNumber(stats?.totalRenewals) : 0}
            </p>
          </div>
        </div>       
      </div>
      ...
      <div className="w-full p-3 border bg-white shodow--box rounded-[10px]">
              ...
              {leaderboardCallLogs.map((row, index) => (
                <tr key={`${row.staff_id}-${index}`} className="bg-white border-b hover:bg-gray-50 border-gray-200">
                  ...
                  <td className="  px-2 py-4 text-center">
                    ₹{row?.new_sales ? formatIndianNumber(row?.new_sales) : 0}
                  </td>

                  <td className="  px-2 py-4 text-center">
                    ₹{row?.renewals ? formatIndianNumber(row?.renewals) : 0}
                  </td>
                  ...
                  <td className=" px-2 py-4 text-center">
                    <div className="flex items-center justify-center">
                        ...
                        >
                        <Link
                            to={`/leaderboard/pt-report/pt-revenue/${row.staff_id}?startDate=${startDate}&endDate=${endDate}`}
        ...
        <Pagination
          ...
          onPageChange={(newPage) => {
            setPage(newPage);
            fetchPtRevenueReportLeaderboard(newPage);
          }}
        />
      </div>
    </div>
