...
const WhatsappTemplate = () => {
  ...
  const fetchServiceList = async (clubId = null) => {
    try {
      ...
      const data = filterActiveItems(
        response.data?.data || response.data?.data || [],
      );
      setServiceList(data);
      const map = data.reduce(
        (acc, item) => ({ ...acc, [String(item.id)]: item.name }),
        {},
      );
      setServiceMap(map);
      return data;
  ...
  const fetchEmailAutomationReport = async (currentPage = page) => {
    try {
      ...
      const response = await authAxios().get("/whatsapp/campaign/list", { params });
      const data = response.data?.data || [];
    ...
    if (item?.membership_expiry_from && item?.membership_expiry_to)
      elements.push(
        <span key="membership_expiry" className="block">
          {formatAutoDate(item.membership_expiry_from)} to{" "}
          {formatAutoDate(item.membership_expiry_to)}
        </span>,
      );
    if (item?.lead_source)
      elements.push(
